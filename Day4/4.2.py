...
for room in key.split("\n"):

    # Convert 3 digits to Integer and store as sectorID
    sectorID = int(room[-10:-7])

    movement = sectorID % 26
    alphabet = string.ascii_lowercase * movement
    dict = {x: y for x, y in zip(alphabet[:26], alphabet[movement:26 + movement])}

    # Convert characters based on movement
    conversion = ""
    for letter in room:
        try:
    ...
    # Stop iteration if 'northpole' is found
    if "northpole" in conversion:
        print(sectorID)
        break
